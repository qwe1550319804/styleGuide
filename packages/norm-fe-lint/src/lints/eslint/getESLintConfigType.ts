 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-norm/index
 * @returns eslint-config-norm/react
 * @returns eslint-config-norm/typescript/index
 * @returns eslint-config-norm/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-norm/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
